...
def modify_html(
    content: bytes, images_dir: str, embed_images: bool
) -> tuple[bytes, int, int]:
    """
    ...
    for index, image in enumerate(images, start=1):
        ...
        try:
            ...
            result = get_image_from_url(
                image_link,
                state.get("default_image_format", "WEBP"),
            ...
            if embed_images:
                ...
                with Path(image_path).open("wb") as img_file:
                    img_file.write(image_content)

                # Use a relative path inside the zip (NOT the temp path)
                zip_image_ref = str(PurePosixPath("images") / filename)
...
def process_zip(input_zip_path: str, zip_embed_images: bool = False):
    ...
    if not has_fichub_attribution(input_zip_path):
        print(
            "This is not a FicHub Zipped HTML File (Did you unpack a FicHub epub and pass it in?)"
        )
        return

    zip_base_name = Path(Path(input_zip_path).name).stem
        ...
        with tempfile.TemporaryDirectory(prefix="ficimage_") as images_dir:
            ...
            with zipfile.ZipFile(output_zip_path, "w") as zf:
                ...
                for file_path in Path(images_dir).iterdir():
                    zf.write(
                        str(file_path),
                        arcname=str(PurePosixPath("images") / file_path.name),
                    )
